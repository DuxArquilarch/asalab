...
_PROJECT_DIR = os.path.join(_HERE, "Dark Wing Project")
_DB_PATH = os.path.join(_HERE, "database.py")
if not os.path.exists(_DB_PATH):
    _DB_PATH = os.path.join(_PROJECT_DIR, "database.py")
...
def _load_db():
    if not os.path.exists(_DB_PATH):
        raise FileNotFoundError(
            "database.py nao encontrado. Coloque geo_visualizer.py dentro da pasta "
            "'Dark Wing Project' ou mantenha a pasta 'Dark Wing Project' ao lado dele."
        )
...
from analise_ra_calc import (gerar_coord_naca as _gerar_coord_naca_orig,
                             gerar_silhueta_asat,
                             _refresh_airfoils, AIRFOILS_AVAILABLE)

# ── perfis customizados injetados por Dark_Wing ───────────────────
_CUSTOM_PROFILES: dict = {}   # name → pts  (Nx2, normalizado 0-1)

def _pts_to_xyl(pts: np.ndarray):
    """Converte polígono fechado (Nx2, x∈[0,1]) para (x, yu, yl)."""
    pts = pts.copy()
    if np.allclose(pts[0], pts[-1]):
        pts = pts[:-1]
    x_min, x_max = pts[:, 0].min(), pts[:, 0].max()
    # re-normaliza por precaução
    chord = x_max - x_min
    if chord > 0:
    ...
    for i in range(n_bins):
        ...
        if np.any(mask):
            xu.append((bins[i] + bins[i + 1]) / 2.0)
            yu.append(float(pts[mask, 1].max()))
            yl.append(float(pts[mask, 1].min()))
    return np.array(xu), np.array(yu), np.array(yl)

def gerar_coord_naca(nome: str):
    """Wrapper: usa perfil customizado se disponível, senão chama original."""
    if nome in _CUSTOM_PROFILES:
        return _pts_to_xyl(_CUSTOM_PROFILES[nome])
    return _gerar_coord_naca_orig(nome)
...
def _corda_local(forma: str, y_pos: float, b: float, c_root: float):
    semi_b = b / 2.0
    eta = min(max(y_pos / semi_b, 0.0), 1.0) if semi_b > 0 else 0.0
    forma_l = str(forma).lower()
    if "retang" in forma_l:
        return c_root
    if "ptica" in forma_l or "elliptic" in forma_l:
        return c_root * np.sqrt(max(0.0, 1.0 - eta**2))
    if "delta" in forma_l:
        return c_root * max(0.0, 1.0 - eta)
    return c_root
...
def launch_geo_visualizer(parent_cfg: dict = None):
    ...
    _load_db()
    _refresh_airfoils()
    perfis_disponiveis = list(AIRFOILS_AVAILABLE)

    # ── injeta perfil externo vindo do Dark_Wing ───────────────────
    custom_name = None
    if parent_cfg and parent_cfg.get("airfoil_pts") is not None:
        custom_name = parent_cfg.get("airfoil_name", "Custom")
        _CUSTOM_PROFILES[custom_name] = np.asarray(parent_cfg["airfoil_pts"])
        if custom_name not in perfis_disponiveis:
            perfis_disponiveis.insert(0, custom_name)
    ...
    if custom_name and custom_name in perfis_disponiveis:
        lb_perfis.selection_set(perfis_disponiveis.index(custom_name))
    elif parent_cfg and parent_cfg.get("perfis_sel"):
    ...
    def _sync_geo_info(*_):
        try:
            c = float(c_var.get())
            b = float(b_var.get())
            s = b * c
            ar = b ** 2 / s if s > 0 else 0.0
            geo_info_var.set(f"RA={ar:.2f}   S={s:.3f}m²")
        except Exception:
            geo_info_var.set("—")
    for _v in (c_var, b_var):
        _v.trace_add("write", _sync_geo_info)
    _sync_geo_info()
    ...
    def _render(*_):
        ...
        with plt.style.context(PLT_STYLE):
            ...
            for idx, perfil in enumerate(perfis):
                ...
                # ── vista 3D pseudo ────────────────────────────────
                if ax_3d:
                    offsets = np.linspace(0, b_real * 0.5, 14)
                    z_min = 0.0
                    z_max = 0.0
                    for yoff in offsets:
                        c_local = _corda_local(asat, yoff, b_real, c_real)
                        xc_local = x * c_local
                        yuc_local = yu * c_local
                        ylc_local = yl * c_local
                        z_min = min(z_min, float(np.min(ylc_local)))
                        z_max = max(z_max, float(np.max(yuc_local)))
                        denom = b_real * 0.5 if b_real > 0 else 1.0
                        alpha3 = 0.35 + 0.45 * (yoff / denom)
                        ax_3d.plot(xc_local, [yoff] * len(xc_local), yuc_local,
                                   color=cor, linewidth=0.9, alpha=alpha3)
                        ax_3d.plot(xc_local, [yoff] * len(xc_local), ylc_local,
                                   color=cor, linewidth=0.9, alpha=alpha3)
                    ax_3d.set_xlim(0, max(c_real, 1e-6))
                    ax_3d.set_ylim(0, max(b_real * 0.5, 1e-6))
                    z_pad = max((z_max - z_min) * 0.20, c_real * 0.02, 1e-3)
                    ax_3d.set_zlim(z_min - z_pad, z_max + z_pad)
                    try:
                        ax_3d.set_box_aspect((max(c_real, 1e-6), max(b_real * 0.5, 1e-6), max(z_max - z_min + 2*z_pad, 1e-6)))
                    except Exception:
                        pass
    ...
    def _export():
        ...
        _reports = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "Reports")
        _os.makedirs(_reports, exist_ok=True)
