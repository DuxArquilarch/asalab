...
def interpolar_coeficientes(re_alvo, dados_perfil):
    """
    Interpolação linear de (a0_2d, αL0, Cd0 [, Cm, XCp]).
    Retrocompatível: tuplas com 3 ou 5 elementos.
    White §7.1.
    """
    ...
    # Garante 5 elementos: completa Cm=-0.05, XCp=0.25 se ausentes
    if raw.size == 3:
        raw = np.append(raw, [-0.05, 0.25])
    return raw  # (a0_2d, αL0, cd0, cm_med, xcp_med)
...
def calcular_asa(v, c, b, peso_kg, perfis_sel, asat_sel):
    ...
    for perfil in perfis_sel:
        ...
        # Inclinação de asa finita (Lifting Line, White §8.4)
        ...
        # Cm e XCp da asa finita
        cm_asa, xcp_asa = calcular_cm_xcp(
            alphas, cl_asa, cm0_2d, xcp0_2d, AR, e_oswald
        )
...
#   §8.1   Centro de pressão — XCp = 0.25 − Cm/CL                   #
